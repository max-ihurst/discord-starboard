...
export default class StarLeaderboardCommand implements Command {
    ...
    public async execute(interaction: CommandInteraction): Promise<void> {
        const stars = this.client.stars.cache.filter(
            (s) => s.guild == interaction.guild?.id
        );
        ...
        if (!members || !stars.size) {
            interaction.reply({
                content: 'There are no starred messages in this guild!',
                ephemeral: true,
            });
            return;
        }
        ...
        const postion =
            leaderboard.findIndex(
                (u) => u.username == interaction.member?.user.username
            ) + 1;
                ...
                new MessageEmbed()
                    ...
                    .setDescription(
                        [
                            ...
                            postion > 0
                                ? 'You are - ' +
                                  bold(postion + ordinal.indicator(postion))
                                : null,
                        ].join('\n')
                    ),
            ],
